...
$sql = "SELECT * FROM reservation WHERE NIK = '$NIK' AND (status = 'Pending' OR status = 'Paid')";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['cancel_reservation_id'])) {
        ...
        if ($conn->query($update_sql) === TRUE) {
            ...
            $insert_sql = "INSERT INTO history SELECT * FROM reservation WHERE id = '$reservation_id' AND status = 'Cancelled'";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            if ($row["status"] == "Pending") {
                ...
                echo "<a href='cancel_reservation.php?id=" . $row["id"] . "'>Cancel Reservation</a>";
                echo "</form>";
            }
            echo "</div>";
        }
