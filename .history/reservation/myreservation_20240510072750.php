...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['cancel_reservation_id'])) {
        ...
        // Move the reservation to history table
        $insert_sql = "INSERT INTO history (id_reservation, NIK, fname, lname, email, phone, troom, bed, nroom, cin, cout, status, nodays, total_cost) SELECT id, NIK, fname, lname, email, phone, troom, bed, nroom, cin, cout, status, nodays, total_cost FROM reservation WHERE id = '$reservation_id'";
        if ($conn->query($insert_sql) === TRUE) {
            echo "Reservation moved to history successfully.";
        } else {
            echo "Error moving reservation to history table: " . $conn->error;
        }
    }
}
