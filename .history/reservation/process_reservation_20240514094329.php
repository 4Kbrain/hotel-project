...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($insert_reservation_stmt->execute()) {
        ...
        if ($get_id_kamar_result->num_rows > 0) {
            ...
            $update_kamar_sql = "UPDATE kamar SET NIK = ?, room_status = 'Pending', id_reservation = ? WHERE id_kamar = ?";
            ...
            if ($update_kamar_stmt->execute()) {
                ...
                if ($update_reservation_stmt->execute()) {
                    ...
                    $insert_history_sql = "INSERT INTO history (id_reservation, id_kamar, NIK, fname, lname, email, phone, troom, bed, nroom, cin, cout, nodays, total_cost, status, kembalian)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    ...
                    echo '<script>window.location.replace("reservation_form></script>'
                }
            } else {
