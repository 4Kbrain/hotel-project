...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($reservation_result->num_rows > 0) {
        ...
        if ($payment >= $total_cost) {
            ...
            $insert_transaction_sql = "INSERT INTO transactions (NIK, nama, cin, cout, payment, total_cost, `change`) 
                                       SELECT NIK, CONCAT(fname, ' ', lname) AS nama, cin, cout, '$payment' AS payment, total_cost, '$change' AS `change`
            ...
            echo "<script>alert("Insufficient payment. Payment should be at least $total_cost";
        }
    } else {
