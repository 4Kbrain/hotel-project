...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($total_cost !== null) {
        ...
        if ($payment >= $total_cost) {
            ...
            $stmt_update_kamar = $con->prepare("UPDATE kamar SET room_status='In Use' WHERE NIK = (SELECT NIK FROM reservation WHERE id=?)");
            $stmt_update_kamar->bind_param("i", $id);
            $stmt_update_kamar->execute();

            // Update history status to Booked and calculate kembalian using join
            $stmt_update_history = $con->prepare("UPDATE history h JOIN reservation r ON h.NIK = r.NIK JOIN kamar k ON r.NIK = k.NIK SET h.status='Booked', h.kembalian=h.kembalian-? WHERE r.id=? AND k.id_kamar=r.id_kamar");
